...
public class Log_Item : MonoBehaviour
{
    ...
    void Start()
    {
        TreesManager.instance.items.Add(this);
        id = TreesManager.instance.items.Count;
    ...
    void Update()
    {
        ...
        if (picked)
        {
            ...
            if (distVect.magnitude < 0.5f)
            {
                ...
                {
                    target.GetComponent<PlayerController>().WoodInventory++;
                }
                else if (target.GetComponent<PlayerShell>())
                {
                    target.GetComponent<PlayerShell>().WoodInventory++;
                }

                if(target == PlayerController.instance.gameObject)
                GUIManager.instance.UpdateInventory();

                TreesManager.instance.items.Remove(this);
